...
This download script will be served from https://storage.googleapis.com/data.yt8m.org/download.py
The partitions are 1/{frame_level,video_level}/{train,validate,test}
...
if __name__ == '__main__':
  #os.environ['partition'] = sys.argv[2]
  #os.environ['mirror'] = sys.argv[3]
  ...
  if 'mirror' not in os.environ:
    ...
    exit(1)
  #print(os.environ['partition'], os.environ['shard'])
  ...
  plan_url = 'https://storage.googleapis.com/data.yt8m.org/{}/download_plans/{}_{}.json'.format(
  ...
  for f in files:
    ...
    download_url = 'http://storage.googleapis.com/%s.data.yt8m.org/%s/%s' % (mirror, partition, f)
